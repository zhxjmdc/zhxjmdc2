...
class ArticletypeModel extends RelationModel{
	 ...
	 * @param  int   $aid    初始值为第一条父级评论cid，表示从根类开始查找
	...
	public function get_type_article($aid,&$array = array()){
		...
		$data = $article_type->where('pid ='.$aid)->select();
		
		if(empty($data)){
			// 叶子节点
			$data_son = $article_type->relation('article')->where('aid ='.$aid)->select();
			...
			// 非叶子节点
			$data_parent = $article_type->relation('article')->where('aid ='.$aid)->select();
	...
	public function get_sider_type(){
		$article_type = D('Articletype');
		$map['pid'] = array('NEQ','0');
        $type_data = $article_type->where($map)->relation('article')->select();
